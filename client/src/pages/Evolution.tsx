...
export default function Evolution({ onTabChange, hideHeader }: EvolutionProps) {
 ...
 <ResponsiveContainer width="100%" height={300}><LineChart data={generalProgress}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis
 dataKey="date"
 tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }}
 tickFormatter={date => new Date(date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
 /><YAxis tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }} /><Tooltip
 contentStyle={{
 ...
 <ResponsiveContainer width="100%" height={300}><LineChart data={exerciseProgress}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis
 dataKey="date"
 tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }}
 tickFormatter={date => new Date(date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
 /><YAxis tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }} /><Tooltip
