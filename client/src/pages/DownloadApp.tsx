...
export default function DownloadApp() {
 ...
 .da-hero {
 border: 1px solid #1a1628; border-radius: 6px; padding: 24px;
 ...
 .da-plat-card {
 border: 1px solid #1c1928; border-radius: 6px; background: #111019;
 ...
 .da-collapse {
 border: 1px solid #18152a; border-radius: 6px;
 ...
 .da-qr-card { border: 1px solid #1a1628; border-radius: 6px; margin: 0 16px 12px; background: #0c0a17; padding: 20px; }
 ...
 border: 1px solid #1a1628; border-radius: 6px; margin: 0 16px 16px;
 ...
 {/* ── HERO ── */}
 <div className="da-hero"><div className="da-hero-tag">PROGRESSIVE WEB APP</div><h2>Acesso instantâneo da tela inicial</h2><p>
 Instale o Ascend como um app nativo. Sem loja, sem downloads pesados —
 funciona em qualquer dispositivo com um toque.
 </p><div className="da-hero-actions"><button className="da-btn-main" onClick={handleInstall}><Download size={13} />
 Instalar Agora
 </button><button className="da-btn-sec" onClick={copyToClipboard}>
 {copied ? <Check size={13} /> : <Copy size={13} />}
 Copiar link
 </button></div></div>

 {/* ── GUIA DE INSTALAÇÃO (lado a lado) ── */}
 <span className="da-label">Guia de instalação</span><div className="da-plat-row">
 ...
 {isInstallable ? (
 androidSteps.map((s) => (
 <div className="da-step" key={s.n}><span className="da-step-n">{s.n}</span><div><div className="da-step-title">{s.title}</div><div className="da-step-sub">{s.sub}</div></div></div>
 ))
 ) : (
 <>
 {[
 ...
 <div className="da-collapse"><div className="da-collapse-header" onClick={() => setStatusOpen((p) => !p)}><div className="da-collapse-left"><div className="da-collapse-dot" />
 ...
 /></div>
 {statusOpen && (
 <div className="da-collapse-body"><div className="da-debug-grid"><div className="da-debug-item"><b>isInstallable </b><span className={isInstallable ? "da-v-true" : "da-v-false"}>{String(isInstallable)}</span></div><div className="da-debug-item"><b>isInstalled </b><span className={isInstalled ? "da-v-true" : "da-v-false"}>{String(isInstalled)}</span></div><div className="da-debug-item"><b>beforeInstallPrompt </b><span className={beforeInstallPromptReceived ? "da-v-true" : "da-v-false"}>{String(beforeInstallPromptReceived)}</span></div><div className="da-debug-item"><b>serviceWorker </b><span className={serviceWorkerRegistered ? "da-v-true" : "da-v-false"}>
 {serviceWorkerRegistered ? "registrado" : "pendente"}
 </span></div><div className="da-debug-item"><b>displayMode </b><span>{displayModeStandalone ? "standalone" : "browser"}</span></div><div className="da-debug-item"><b>online </b><span className={isOnline ? "da-v-true" : "da-v-false"}>{String(isOnline)}</span></div></div></div>
 )}
 </div>

 {/* ── QR / CELULAR ── */}
 <span className="da-label">Instalar no celular</span><div className="da-qr-card"><div className="da-qr-block"><div className="da-qr-frame"><img src={qrUrl} alt="QR Code Ascend" /></div><div className="da-qr-info"><h3>Escaneie para instalar no celular</h3><p>
 Aponte a câmera para o QR ou acesse o link direto no navegador do
 seu dispositivo.
 </p><div className="da-url-row"><code>{APP_URL.replace("https://", "")}</code><button className="da-copy-btn" onClick={copyToClipboard} title="Copiar">
 {copied ? <Check size={12} color="#34D399" /> : <Copy size={12} />}
 </button></div><div className="da-check-list"><div className="da-check-item">Funciona em qualquer navegador mobile</div><div className="da-check-item">Instala direto da tela inicial sem loja</div><div className="da-check-item">Compartilhe com amigos e família</div></div></div></div></div>

 {/* ── POR QUE INSTALAR ── */}
 <span className="da-label">Por que instalar?</span><div className="da-benefits">
 {benefits.map((b, i) => {
 const Icon = b.icon;
 return (
 ...
 {/* ── FOOTER ── */}
 <div className="da-footer"><div><p>Compartilhe o Ascend</p><code>{APP_URL}</code></div><button className="da-btn-main" onClick={copyToClipboard} style={{ fontSize: 12, padding: "9px 16px" }}>
